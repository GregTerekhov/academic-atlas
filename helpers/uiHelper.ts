export const getDropdownTriggerStyles = (isOptionSelected: boolean, isDropdownOpen: boolean) => {
  return `${isOptionSelected ? 'border-none bg-accent-lightGradient dark:bg-accent-darkGradient' : 'border-accentPrimary dark:border-accentSecondary-darker bg-whiteBase dark:bg-darkBase'} flex h-10 w-full items-center justify-between border border-solid px-2 hocus:border-transparent hocus:outline-none hocus:ring-[2px] hocus:ring-accentPrimary dark:hocus:ring-accentSecondary md:h-12 md:px-4 ${isDropdownOpen ? 'rounded-t-lg' : 'rounded-lg'}`;
};
...
export const getDropdownLabelStyles = (isOptionSelected: boolean) => {
  return isOptionSelected
    ...
    : 'text-sm max-md:leading-130 md:text-base lg:text-big';
};
